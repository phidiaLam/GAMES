...
{
    auto v = t.toVector4();
    float bound_x_min = std::min(std::min(v[0].x(), v[1].x()), v[2].x());
    float bound_y_min = std::min(std::min(v[0].y(), v[1].y()), v[2].y());
    ...
    for (int x = (int)bound_x_min; x <= (int)bound_x_max; x++)
    {
        for (int y = (int)bound_y_min;y <= (int)bound_y_max; y++)
        {
            ...
            auto interpolated_viewpos = interpolate(alpha, beta, gamma, view_pos[0], view_pos[1], view_pos[2], 1.0);
